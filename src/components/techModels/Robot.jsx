...
export default function Robot({setRobotRef, props}) {
  const group = React.useRef()
  const { nodes, materials, animations } = useGLTF('/models/robot.glb')
  const { actions, names } = useAnimations(animations, group);

  const wave_color = new THREE.MeshPhongMaterial({
    color: "#6D28D9"
  });

  const ear_color = new THREE.MeshPhongMaterial({
    color: "#6D28D9"
  });

  const face_color = new THREE.MeshPhongMaterial({
    color: "#6D28D9"
  });
  ...
  return (
    <group ref={group} {...props} dispose={null} position={[0, -0.5, 3.5]} >
      <group name="Sketchfab_Scene" ref={setRobotRef} rotation={[.2, -1.2, 0]} position={[2.5, 0, -9]} >
              ...
              <group name="RootNode">
                <group name="Robot_Origin" position={[0, 9.763, 0]} rotation={[-Math.PI / 2, 0, 0]} scale={100}>
                  ...
                  <group name="Ears"  position={[0, 0, 2.967]}>
                    <mesh name="Ears_Black_Matt_0" geometry={nodes.Ears_Black_Matt_0.geometry} material={ear_color} />
                  </group>
                  ...
                  <group name="Robot" position={[0, 0, 0.051]}>
                    ...
                    <mesh name="Robot_Black_Matt_0" geometry={nodes.Robot_Black_Matt_0.geometry} material={face_color} />
                  </group>
