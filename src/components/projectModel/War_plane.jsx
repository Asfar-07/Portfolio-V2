...
export function WarPlane(props) {
  ...
  useFrame((_, delta) => {
    ...
    if (pointer.current) {
      ...
      if(speed > 20){
        pointer.current.rotation.y=0
      }
      // Smooth interpolation
     pointer.current.scale.x = THREE.MathUtils.lerp(
       pointer.current.scale.x,
       targetScale,
       0.03,
     );
     pointer.current.scale.y = THREE.MathUtils.lerp(
       pointer.current.scale.y,
       targetScale,
       0.03,
     );
     pointer.current.scale.z = THREE.MathUtils.lerp(
       pointer.current.scale.z,
       targetScale,
       0.03,
     );

      pointer.current.position.y = THREE.MathUtils.lerp(
        pointer.current.position.y,
        targetY,
        0.03,
      );

      pointer.current.rotation.x = THREE.MathUtils.lerp(
        pointer.current.rotation.x,
        rotationX,
        0.03,
      );
    ...
    if (propellerRef.current) {
      propellerRef.current.rotation.z += delta * currentSpeed.current;
    }
  });
  const { nodes, materials, animations } = useGLTF("/models/war_plane.glb");
    ...
    >
      <group name="Sketchfab_Scene">
        ...
        >
          ...
          >
            <group name="Object_2">
              <group name="RootNode" rotation={[0, 0, 0]} >
                <group
                  name="polySurface172"
                  ...
                  scale={0.9}
                >
                  <group name="polySurface171" position={[0, -0.57, 14.211]} rotation={[0, 0, -2.64]} ref={propellerRef}>
...
useGLTF.preload('/models/war_plane.glb')
